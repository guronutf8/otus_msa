...
func main() {
	...
	muxRouter.HandleFunc("/GetBasket", getBasket).Methods(http.MethodGet)
...
func getBasket(w http.ResponseWriter, r *http.Request) {
	var reqData OrderPB.GetBasketRequest
	if ok := request.HttpReqToObj(w, r, &reqData); !ok {
		return
	}

	basket, err := db.GetCurrentBasket(r.Context(), reqData.GetUser())
	if err != nil {
		slog.Error("GetCurrentBasket", err)
	}
	request.Common{Status: true, Message: basket}.WriteResponse(w)
}
...
func order(w http.ResponseWriter, r *http.Request) {
	...
	wasChange, err := db.ChangeStatus(r.Context(), orderData.GetUser(), orderData.GetBasket())
	if err != nil {
		slog.Error("change status", "err", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}
